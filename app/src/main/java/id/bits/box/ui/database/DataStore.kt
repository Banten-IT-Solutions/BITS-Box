...
import id.bits.box.CONNECTION_TEST_URL
import id.bits.box.GroupType
import id.bits.box.IPv6Mode
import id.bits.box.Key
import id.bits.box.TunImplementation
...
import id.bits.box.database.preference.PublicDatabase
import id.bits.box.database.preference.RoomPreferenceDataStore
import id.bits.box.ktx.boolean
import id.bits.box.ktx.int
import id.bits.box.ktx.long
import id.bits.box.ktx.parsePort
import id.bits.box.ktx.string
import id.bits.box.ktx.stringToInt
import id.bits.box.ktx.stringToIntIfExists
import id.bits.box.TempDatabase
...
object DataStore : OnPreferenceDataStoreChangeListener {
    ...
    fun currentGroupId(): Long {
        ...
        val groupId = BitsBoxDatabase.groupDao.createGroup(ProxyGroup(ungrouped = true))
        selectedGroup = groupId
        return groupId
    }
    ...
    fun currentGroup(): ProxyGroup {
        ...
        if (currentSelected > 0L) {
            group = BitsBoxDatabase.groupDao.getById(currentSelected)
        }
        if (group != null) return group
        val groups = BitsBoxDatabase.groupDao.allGroups()
        if (groups.isEmpty()) {
            group = ProxyGroup(ungrouped = true).apply {
                id = BitsBoxDatabase.groupDao.createGroup(this)
            }
        } else {
            group = groups[0]
        }
    ...
    var allowInsecureOnRequest by configurationStore.boolean(Key.ALLOW_INSECURE_ON_REQUEST) { true }
    var networkChangeResetConnections by configurationStore.boolean(Key.NETWORK_CHANGE_RESET_CONNECTIONS) { true }
    var wakeResetConnections by configurationStore.boolean(Key.WAKE_RESET_CONNECTIONS) { false }

    //

    var isExpert by configurationStore.boolean(Key.APP_EXPERT) { true }
    ...
    var showGroupInNotification by configurationStore.boolean("showGroupInNotification") { false }
    ...
    private var storedRulesVariant by configurationStore.stringToInt(Key.RULES_VARIANT)
    var rulesVariant: Int
        get() {
            val storedVariant = storedRulesVariant
            return storedVariant.coerceIn(0, 1).also { if (it != storedVariant) storedRulesVariant = it }
        }
        set(value) { storedRulesVariant = value.coerceIn(0, 1) }
    // Menandai variant terakhir yang berhasil di-download untuk tiap aset.
    // Dipakai agar tombol update tahu kapan perlu men-download ulang saat
    // variant berubah. Dilacak per-file karena geoip & geosite bisa di-update
    // secara terpisah (mis. ganti variant di tengah proses update).
    var geoipDownloadedVariant by configurationStore.int(Key.GEOIP_DOWNLOADED_VARIANT) { 0 }
    var geositeDownloadedVariant by configurationStore.int(Key.GEOSITE_DOWNLOADED_VARIANT) { 0 }
    var logLevel by configurationStore.stringToInt(Key.LOG_LEVEL)
    var logBufSize by configurationStore.int(Key.LOG_BUF_SIZE) { 1024 }
    ...
    var meteredNetwork by configurationStore.boolean(Key.METERED_NETWORK) { false }
    var proxyApps by configurationStore.boolean(Key.PROXY_APPS) { false }
    ...
    var connectionTestURL by configurationStore.string(Key.CONNECTION_TEST_URL) { "http://cp.cloudflare.com/" }
    var connectionTestConcurrent by configurationStore.int("connectionTestConcurrent") { 3 }
    var alwaysShowAddress by configurationStore.boolean(Key.ALWAYS_SHOW_ADDRESS) { false }
    ...
    var globalAllowInsecure by configurationStore.boolean(Key.GLOBAL_ALLOW_INSECURE) { true }
