...
import id.bits.box.NativeInterface
import id.bits.box.net.LocalResolverImpl
import id.bits.box.utils.JavaUtil
import id.bits.box.utils.cleanWebview
import java.io.File

class BitsBoxApp : Application(), WorkConfiguration.Provider {

    // WorkManager auto-init (androidx.startup) is disabled in the manifest, so it must be
    // initialized through Configuration.Provider (required for multi-process + RemoteWorkManager).
    ...
    companion object {
        ...
        lateinit var application: BitsBoxApp
        ...
        fun startService() = ContextCompat.startForegroundService(
            application, Intent(application, BitsBoxConnection.serviceClass)
        )
