...
import id.bits.box.databinding.LayoutGroupItemBinding
...
class GroupFragment : ToolbarFragment(R.layout.layout_group),
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        setNavigationIcon(R.drawable.ic_baseline_view_list_24)
        ...
        when (item.itemId) {
            ...
            R.id.action_update_all -> {
                requireContext().confirmDialog(
                    ...
                    iconColorRes = R.color.dialog_icon_primary,
                    title = getString(R.string.confirm),
                    message = getString(R.string.update_all_subscription),
                    positive = getString(R.string.yes),
                    negative = getString(R.string.no),
                ) {
                    BitsBoxDatabase.groupDao.allGroups()
        ...
        registerForActivityResult(ActivityResultContracts.CreateDocument("text/plain")) { data ->
            if (data != null) {
                runOnDefaultDispatcher {
                    val profiles = BitsBoxDatabase.proxyDao.getByGroup(selectedGroup.id)
    ...
    inner class GroupAdapter : RecyclerView.Adapter<GroupHolder>(),
        ...
        suspend fun reload() {
            val groups = BitsBoxDatabase.groupDao.allGroups().toMutableList()
            if (groups.size > 1 && BitsBoxDatabase.proxyDao.countByGroup(groups.find { it.ungrouped }!!.id) == 0L) groups.removeAll { it.ungrouped }
            groupList.clear()
        ...
        fun commitMove() = runOnDefaultDispatcher {
            updated.forEach { BitsBoxDatabase.groupDao.updateGroup(it) }
            updated.clear()
        }
        ...
        override suspend fun groupRemoved(groupId: Long) {
            ...
            onMainDispatcher {
                undoManager.flush()
                if (BitsBoxDatabase.groupDao.allGroups().size <= 2) {
                    runOnDefaultDispatcher {
                        reload()
                    }
                } else {
    ...
    inner class GroupHolder(binding: LayoutGroupItemBinding) :
            ...
            when (item.itemId) {
                ...
                R.id.action_export_clipboard -> {
                    runOnDefaultDispatcher {
                        ...
                        val links = profiles.joinToString("\n") { it.toStdLink(compact = true) }
                        onMainDispatcher {
                            BitsBoxApp.trySetPrimaryClip(links)
                ...
                R.id.action_clear -> {
                    requireContext().confirmDialog(
                        icon = R.drawable.baseline_delete_sweep_24,
                        iconColorRes = R.color.material_red_500,
                        title = getString(R.string.confirm),
                        message = getString(R.string.clear_profiles_message),
                        positive = getString(R.string.yes),
        ...
        fun bind(group: ProxyGroup) {
            ...
            // Type chip: neutral, same style as profile chips (colors come from XML)
            groupType.text = getString(
                if (proxyGroup.type == GroupType.SUBSCRIPTION) R.string.subscription else R.string.group_basic
            )
            ...
            optionsButton.setOnClickListener {
                ...
                val popup = PopupMenu(requireContext(), it, Gravity.NO_GRAVITY, 0, R.style.Widget_BITSBox_PopupMenu)
                popup.menuInflater.inflate(R.menu.group_action_menu, popup.menu)
                popup.setForceShowIcon(true)

                // Hide QR Code and Link actions for basic groups (only for subscription groups)
            ...
            if (subscription != null && subscription.bytesUsed > 0L) { // SIP008 & Open Online Config
                ...
                groupTraffic.setChipBackgroundColor(
                    ColorStateList.valueOf(requireContext().getColour(R.color.material_cyan_500))
                )
            ...
            groupUser.isGone = groupUser.text.isBlank()

            runOnDefaultDispatcher {
                val size = BitsBoxDatabase.proxyDao.countByGroup(group.id)
                onMainDispatcher {
                    groupStatus.text = size.toString()
                }
