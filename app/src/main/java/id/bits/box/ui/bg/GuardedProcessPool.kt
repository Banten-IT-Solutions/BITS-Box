...
import id.bits.box.utils.Commandline
...
class GuardedProcessPool(private val onFatal: suspend (IOException) -> Unit) : CoroutineScope {
    private inner class Guard(
        ...
        fun start() {
            process = ProcessBuilder(cmd).directory(BitsBoxApp.application.noBackupFilesDir).apply {
            ...
            try {
                while (true) {
                    thread(name = "stderr-$cmdName") {
                        streamLogger(process.errorStream) { Libcore.bitsBoxLogPrintln("[$cmdName] $it") }
                    }
                    thread(name = "stdout-$cmdName") {
                        streamLogger(process.inputStream) { Libcore.bitsBoxLogPrintln("[$cmdName] $it") }
                ...
                BitsBoxApp.application.applicationScope.launch(Dispatchers.Main) { onFatal(e) }
            } finally {
    ...
    fun start(
        ...
        Guard(cmd, env).apply {
            ...
            BitsBoxApp.application.applicationScope.launch { looper(onRestartCallback) }
        }
        processCount += 1
    }
