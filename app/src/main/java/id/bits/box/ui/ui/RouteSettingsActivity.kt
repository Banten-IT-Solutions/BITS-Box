...
class RouteSettingsActivity(
        ...
        if (savedInstanceState == null) {
            ...
            runOnDefaultDispatcher {
                if (editingId == 0L) {
                    ...
                    val ruleEntity = BitsBoxDatabase.rulesDao.getById(editingId)
                    if (ruleEntity == null) {
                        onMainDispatcher {
                            finish()
                        }
    ...
    suspend fun saveAndExit() {
        ...
        if (!needSave()) {
            onMainDispatcher {
                confirmDialog(
                    ...
                    title = getString(R.string.empty_route),
                    message = getString(R.string.empty_route_notice),
        ...
        if (editingId == 0L) {
            ...
            val entity = BitsBoxDatabase.rulesDao.getById(DataStore.editingId)
            if (entity == null) {
                finish()
                return
            }
    ...
    override fun onCreateOptionsMenu(menu: Menu): Boolean {
        menuInflater.inflate(R.menu.route_config_menu, menu)
        return true
    }
