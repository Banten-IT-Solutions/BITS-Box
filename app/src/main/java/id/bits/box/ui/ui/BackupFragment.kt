...
import id.bits.box.bg.Executable
import id.bits.box.database.*
import id.bits.box.database.preference.KeyValuePair
import id.bits.box.database.preference.PublicDatabase
import id.bits.box.databinding.LayoutBackupBinding
import id.bits.box.databinding.LayoutImportBinding
import id.bits.box.databinding.LayoutProgressBinding
...
class BackupFragment : NamedFragment(R.layout.layout_backup) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        setNavigationIcon(R.drawable.baseline_construction_24)
        ...
        binding.actionExport.setOnClickListener {
            runOnDefaultDispatcher {
                ...
                onMainDispatcher {
                    startFilesForResult(
                        exportSettings, "bitsbox_backup_${SimpleDateFormat("yyyyMMdd-HHmmss", Locale.US).format(Date())}.json"
                    )
                }
            }
        }
        ...
        binding.actionShare.setOnClickListener {
            runOnDefaultDispatcher {
                ...
                app.cacheDir.mkdirs()
                val cacheFile = File(
                    app.cacheDir, "bitsbox_backup_${SimpleDateFormat("yyyyMMdd-HHmmss", Locale.US).format(Date())}.json"
                )
        ...
        val out = JSONObject().apply {
            ...
            if (profile) {
                put("profiles", JSONArray().apply {
                    BitsBoxDatabase.proxyDao.getAll().forEach {
                        put(it.toBase64Str())
                    }
                })

                put("groups", JSONArray().apply {
                    BitsBoxDatabase.groupDao.allGroups().forEach {
                        put(it.toBase64Str())
                    }
                })
            }
            if (rule) {
                put("rules", JSONArray().apply {
                    BitsBoxDatabase.rulesDao.allRules().forEach {
                        put(it.toBase64Str())
                    }
                })
            }
    ...
    fun finishImport(
        ...
        if (profile && content.has("profiles")) {
            ...
            BitsBoxDatabase.proxyDao.reset()
            BitsBoxDatabase.proxyDao.insert(profiles)
            ...
            BitsBoxDatabase.groupDao.reset()
            BitsBoxDatabase.groupDao.insert(groups)
        }
        if (rule && content.has("rules")) {
            ...
            BitsBoxDatabase.rulesDao.insert(rules)
        }
