...
import id.bits.box.fmt.LOCALHOST
import id.bits.box.ktx.*
import id.bits.box.ui.VpnRequestActivity
import id.bits.box.utils.Subnet
...
class VpnService : BaseVpnService(),
    ...
    override fun acquireWakeLock() {
        wakeLock = BitsBoxApp.power.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "bitsbox:vpn")
            .apply { acquire() }
    }
    ...
    override val tag = "BITSBoxVpnService"
...
//        val tunOptions = JSONObject(tunOptionsJson)
        ...
        // interface address, route & MTU are set from the BITS Box GUI config
        val builder = Builder().setConfigureIntent(BitsBoxApp.configureIntent(this))
        ...
        if (proxyApps || needBypassRootUid) {
            ...
            individual.apply {
                // Allow the app itself to use the VPN.
                remove(packageName)
    ...
    fun updateUnderlyingNetwork(builder: Builder? = null) {
        BitsBoxApp.underlyingNetwork?.let {
            builder?.setUnderlyingNetworks(arrayOf(BitsBoxApp.underlyingNetwork))
                ?: setUnderlyingNetworks(arrayOf(BitsBoxApp.underlyingNetwork))
        }
    }
