...
import id.bits.box.fmt.TAG_BYPASS
import id.bits.box.fmt.TAG_PROXY
    ...
    private suspend fun loop() {
        ...
        while (sc.isActive) {
            ...
            if (data.state == BaseService.State.Connected
                && data.binder.callbackIdMap.containsValue(BitsBoxConnection.CONNECTION_ID_MAIN_ACTIVITY_FOREGROUND)
            ) {
                data.binder.broadcast { b ->
                    if (data.binder.callbackIdMap[b] == BitsBoxConnection.CONNECTION_ID_MAIN_ACTIVITY_FOREGROUND) {
