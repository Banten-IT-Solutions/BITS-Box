...
import id.bits.box.database.RuleEntity
import id.bits.box.database.BitsBoxDatabase
import com.google.android.material.chip.Chip
import com.google.android.material.switchmaterial.SwitchMaterial
import id.bits.box.databinding.LayoutRouteItemBinding
...
class RouteFragment : ToolbarFragment(R.layout.layout_route), Toolbar.OnMenuItemClickListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        setNavigationIcon(R.drawable.ic_maps_directions)
                ...
                MaterialAlertDialogBuilder(activity).setTitle(R.string.confirm)
                    ...
                    .setPositiveButton(R.string.yes) { _, _ ->
                        runOnDefaultDispatcher {
                            BitsBoxDatabase.rulesDao.reset()
    ...
    inner class RuleAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>(), ProfileManager.RuleListener, UndoSnackbarManager.Interface<RuleEntity> {
        ...
        fun move(from: Int, to: Int) {
            ...
            val (step, range) = if (from < to) Pair(1, from until to) else Pair(-1, to downTo from)
            for (i in range) {
        ...
        fun commitMove() = runOnDefaultDispatcher {
            if (updated.isNotEmpty()) {
                BitsBoxDatabase.rulesDao.updateRules(updated.toList())
                updated.clear()
                needReload()
            }
        }
        ...
        inner class RuleHolder(binding: LayoutRouteItemBinding) : RecyclerView.ViewHolder(binding.root) {
            ...
            val ruleName = binding.ruleName
            val typeChip = binding.typeChip
            val outboundChip = binding.outboundChip
            val editButton = binding.edit
            val appsInfo = binding.appsInfo
            val matchCount = binding.matchCount
            ...
            fun bind(ruleEntity: RuleEntity) {
                ...
                // Name
                ruleName.text = rule.displayName()
                
                // Type chip - show rule type in Title Case
                val ruleType = when {
                    rule.domains.isNotBlank() && rule.ip.isBlank() -> "Domain"
                    rule.ip.isNotBlank() -> "IP"
                    rule.port.isNotBlank() -> "Port"
                    rule.network.isNotBlank() || rule.protocol.isNotBlank() -> "Network"
                    rule.packages.isNotEmpty() -> "App"
                    else -> "All"
                }
                typeChip.text = ruleType
                
                // Outbound chip - Title Case format
                outboundChip.text = routeOutboundCapitalize(rule.displayOutbound())
                
                // Apps info (optional - only for APP rules)
                if (rule.packages.isNotEmpty()) {
                    appsInfo.text = app.getString(R.string.apps_message, rule.packages.size)
                    appsInfo.visibility = View.VISIBLE
                    matchCount.visibility = View.GONE
                } else {
                    appsInfo.visibility = View.GONE
                    matchCount.visibility = View.GONE
                }
                ...
                deleteButton.setOnClickListener {
                    ...
                    if (pos != RecyclerView.NO_POSITION) {
                        ruleAdapter.remove(pos)
                        undoManager.remove(pos to rule)
                    }
                }
            }

            private fun routeOutboundCapitalize(outbound: String): String {
                return when (outbound.lowercase()) {
                    "proxy" -> "Proxy"
                    "bypass" -> "Bypass"
                    "block" -> "Block"
                    else -> outbound.split(" ").joinToString(" ") { word ->
                        word.ifBlank { "" }.replaceFirstChar { 
