...
object Util {
    ...
    /**
     * Extract substring between two text values
     *
     * @param text  Source text, e.g.: "12345" to extract
     * @param left  Text before (can be null)
     * @param right Text after (can be null)
    ...
    // v2rayN Style - single line encoding
    ...
    fun b64Decode(b: String): ByteArray {
        ...
        // URLSafe base64 requires character replacement
        val str = b.replace("-", "+").replace("_", "/")

        val flags = listOf(
            Base64.DEFAULT,  // multi-line encoding
            Base64.NO_WRAP   // single-line encoding
        )
        ...
        throw IllegalStateException("Cannot decode base64 string")
    }
    ...
    fun tryToSetField(o: Any, name: String, value: Any) {
        try {
            ...
            Logs.w("tryToSetField failed for $name: ${e.message}")
        }
    }
    ...
    fun collapseStatusBar(context: Context) {
        try {
            ...
            Logs.w("collapseStatusBar failed: ${e.message}")
        }
    }
