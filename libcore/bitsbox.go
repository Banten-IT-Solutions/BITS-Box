...
import (
	...
	"github.com/Banten-IT-Solutions/bits-box-core/bitsbox_common"
...
func BITSBoxLogPrintln(s string) {
	log.Println(s)
}

func BITSBoxLogClear() {
	bitsbox_log.LogWriter.Truncate()
}
...
func InitCore(process, cachePath, internalAssets, externalAssets string,
	...
	if1 BITSBoxInterface, if2 BoxPlatformInterface, if3 LocalDNSTransport,
) {
	defer device.DeferPanicToError("InitCore", func(err error) { log.Println(err) })
	isBgProcess = strings.HasSuffix(process, ":bg")

	bitsbox_common.RunMode = bitsbox_common.RunMode_BITSBoxForAndroid
	intfBITSBox = if1
	intfBox = if2
	...
	bitsbox_log.LogWriterDisable = !logEnable
	bitsbox_log.TruncateOnStart = isBgProcess
	bitsbox_log.SetupLog(int(maxLogSizeKb)*1024, filepath.Join(cachePath, "bitsbox.log"))
	...
	// bg process: extract assets synchronously so all files complete
	// before the process can be killed by the system
	if isBgProcess {
		extractAssets()
	}
}
